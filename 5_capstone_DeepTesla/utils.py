...
def mkv_to_mp4(mkv_path, remove_mkv=False):
    # Add to fitting the windows system
    mkv_path = mkv_path.replace('\\', '/')
    ...
    # Change to fit the windows system
    cmd = 'ffmpeg -i {} -c:v copy -c:a libfdk_aac -b:a 128k {}'.format(mkv_path, mp4_path)
...
def get_model():
    ...
    """
    model_path = join_dir(params.model_dir, 'vgg16_add_model.json')
    param_path = join_dir(params.model_dir, 'vgg16_add_model.h5')
